...
@router.post("/spectre")
async def run_spectre(body: dict):
    symbol = body.get("symbol", "AAPL")
    try:
        from signals.spectre import SpectreEngine
        engine = SpectreEngine()
        result = await asyncio.to_thread(engine.analyze, symbol, body.get("timeframe", "1d"))
        return result
    except ImportError:
...
@router.post("/tsfresh")
async def run_tsfresh(body: dict):
    symbol = body.get("symbol", "AAPL")
    try:
        from signals.tsfresh_engine import TSFreshEngine
        engine = TSFreshEngine()
        result = await asyncio.to_thread(engine.extract_features, symbol, body.get("interval", "1d"), body.get("period", 100))
        return result
    except ImportError:
