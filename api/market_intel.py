...
from .market_intel_cache import delete_pattern, get_json, set_json
...
def _get_db_path() -> str:
    global _DB_PATH
    if _DB_PATH is None:
        _DB_PATH = os.getenv("TRADING_ENGINE_MARKET_INTEL_DB_PATH",
                             os.path.join(os.getcwd(), "trading_engine.db"))
    return _DB_PATH
...
def _daily_close_as_of_iso(raw_date: Optional[str]) -> Optional[str]:
    ...
    close_dt = datetime(
        ...
        US_MARKET_CLOSE_TIME.hour, US_MARKET_CLOSE_TIME.minute,
...
def _build_stock_price_metadata(price_as_of: Optional[str], price_source: Optional[str]) -> dict[str, Any]:
    ...
    if parsed_as_of is None:
        return {"price_stale": True, "price_status": "stale", "price_age_seconds": None}
    now_utc = _utc_now()
    ...
    return {"price_stale": stale, "price_status": status, "price_age_seconds": age_seconds}
...
def _fetch_daily_adjusted_series(symbol: str) -> list[dict[str, Any]]:
    ...
    for date_str, values in series.items():
        ...
        try:
            ...
            volume_value = 0.0
        rows.append({"date": date_str, "close": close_value, "volume": volume_value})
...
def _fetch_btc_daily_series() -> list[dict[str, Any]]:
    ...
    for date_str, values in series.items():
        if not isinstance(values, dict):
            continue
        close_value = None
        for key in ("4b. close (USD)", "4a. close (USD)", "4. close"):
            try:
        ...
        if close_value is None:
            continue
        rows.append({"date": date_str, "close": close_value})
    rows.sort(key=lambda row: row["date"], reverse=True)
    return rows
...
def _build_macro_signals() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if btc_return is not None:
        ...
        signals.append({
            "id": "btc_trend", "label": "BTC trend", "label_zh": "BTC 趋势",
            "status": status, "value": round(btc_return, 2), "unit": "%",
            "lookback_days": BTC_MACRO_LOOKBACK_DAYS,
            "explanation": explanation, "explanation_zh": explanation_zh,
            "source": "DIGITAL_CURRENCY_DAILY", "as_of": btc_series[0]["date"],
        })
    if qqq_return is not None:
        ...
        signals.append({
            "id": "qqq_trend", "label": "QQQ trend", "label_zh": "QQQ 趋势",
            "status": status, "value": round(qqq_return, 2), "unit": "%",
            "lookback_days": MACRO_SIGNAL_LOOKBACK_DAYS,
            "explanation": explanation, "explanation_zh": explanation_zh,
            "source": "TIME_SERIES_DAILY_ADJUSTED", "as_of": qqq_series[0]["date"],
        })
    if qqq_return is not None and xlp_return is not None:
        ...
        signals.append({
            "id": "qqq_vs_xlp", "label": "QQQ vs XLP", "label_zh": "QQQ 相对 XLP",
    ...
    if gld_return is not None and uup_return is not None:
        ...
        signals.append({
            "id": "safe_haven_pressure", "label": "Safe-haven pressure", "label_zh": "避险压力",
            "status": status, "value": round(safe_haven_strength, 2), "unit": "%",
            "lookback_days": MACRO_SIGNAL_LOOKBACK_DAYS,
            "explanation": explanation, "explanation_zh": explanation_zh,
            "source": "TIME_SERIES_DAILY_ADJUSTED", "as_of": gld_series[0]["date"],
        })
    ...
    meta = {
        "summary": summary, "summary_zh": summary_zh,
        "defensive_count": defensive_count, "latest_prices": {
    ...
    source = {
        "alpha_vantage_functions": ["TIME_SERIES_DAILY_ADJUSTED", "DIGITAL_CURRENCY_DAILY"],
        "news_dependency": "market_news_snapshots.macro",
    }
    return signals, {
        "verdict": verdict, "bullish_count": bullish_count,
        "total_count": total_count, "meta": meta, "source": source,
    }
...
def _prune_market_news_history(cursor) -> None:
    for category in NEWS_CATEGORY_DEFINITIONS:
        cursor.execute(
            "DELETE FROM market_news_snapshots WHERE category = ? AND id NOT IN ("
            "SELECT id FROM market_news_snapshots WHERE category = ? ORDER BY created_at DESC, id DESC LIMIT ?"
...
def refresh_market_news_snapshots() -> dict[str, Any]:
    ...
    return {"inserted_categories": inserted, "errors": errors, "created_at": created_at}
...
def _load_latest_news_snapshot(category: str) -> Optional[dict[str, Any]]:
    ...
    try:
        cursor.execute(
            "SELECT category, items_json, summary_json, created_at FROM market_news_snapshots WHERE category = ? ORDER BY created_at DESC, id DESC LIMIT 1",
            (category,),
        )
        row = cursor.fetchone()
        if not row:
...
def _prune_macro_signal_history(cursor) -> None:
    cursor.execute(
        "DELETE FROM macro_signal_snapshots WHERE id NOT IN (SELECT id FROM macro_signal_snapshots ORDER BY created_at DESC, id DESC LIMIT ?)",
...
def _prune_etf_flow_history(cursor) -> None:
    cursor.execute(
        "DELETE FROM etf_flow_snapshots WHERE id NOT IN (SELECT id FROM etf_flow_snapshots ORDER BY created_at DESC, id DESC LIMIT ?)",
...
def _prune_stock_analysis_history(cursor) -> None:
    ...
    for symbol in symbols:
        cursor.execute(
            "DELETE FROM stock_analysis_snapshots WHERE symbol = ? AND id NOT IN ("
            "SELECT id FROM stock_analysis_snapshots WHERE symbol = ? ORDER BY created_at DESC, id DESC LIMIT ?"
...
def refresh_macro_signal_snapshot() -> dict[str, Any]:
    ...
    try:
        cursor.execute(
            ...
            (snapshot_key, snapshot["verdict"], snapshot["bullish_count"], snapshot["total_count"],
             json.dumps(signals, ensure_ascii=True), json.dumps(snapshot["meta"], ensure_ascii=True),
             json.dumps(snapshot["source"], ensure_ascii=True), created_at),
        )
    ...
    delete_pattern(_cache_key("overview"))
    return {"verdict": snapshot["verdict"], "bullish_count": snapshot["bullish_count"], "total_count": snapshot["total_count"], "created_at": created_at}
...
def get_macro_signals_payload() -> dict[str, Any]:
    ...
    try:
        cursor.execute(
            "SELECT verdict, bullish_count, total_count, signals_json, meta_json, source_json, created_at FROM macro_signal_snapshots ORDER BY created_at DESC, id DESC LIMIT 1"
        )
        row = cursor.fetchone()
        if not row:
            payload = {"available": False, "verdict": "unavailable", "bullish_count": 0, "total_count": 0, "signals": [], "meta": {}, "source": {}, "created_at": None}
            set_json(cache_key, payload, ttl_seconds=MACRO_SIGNAL_CACHE_TTL_SECONDS)
            return payload
        payload = {
            "available": True, "verdict": row["verdict"], "bullish_count": row["bullish_count"],
            "total_count": row["total_count"], "signals": json.loads(row["signals_json"] or "[]"),
            "meta": json.loads(row["meta_json"] or "{}"), "source": json.loads(row["source_json"] or "{}"),
            "created_at": row["created_at"],
        }
        set_json(cache_key, payload, ttl_seconds=MACRO_SIGNAL_CACHE_TTL_SECONDS)
        return payload
...
def refresh_etf_flow_snapshot() -> dict[str, Any]:
    ...
    try:
        cursor.execute(
            ...
            (snapshot_key, json.dumps(summary, ensure_ascii=True), json.dumps(etfs, ensure_ascii=True), created_at),
        )
    ...
    delete_pattern(_cache_key("overview"))
    return {"direction": summary["direction"], "tracked_count": summary["tracked_count"], "created_at": created_at}
...
def get_etf_flows_payload() -> dict[str, Any]:
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT summary_json, etfs_json, created_at FROM etf_flow_snapshots ORDER BY created_at DESC, id DESC LIMIT 1")
        row = cursor.fetchone()
        if not row:
            payload = {"available": False, "summary": {}, "etfs": [], "created_at": None, "is_estimated": True}
            set_json(cache_key, payload, ttl_seconds=ETF_FLOW_CACHE_TTL_SECONDS)
            return payload
        summary = json.loads(row["summary_json"] or "{}")
        payload = {
            "available": True, "summary": summary,
            "etfs": json.loads(row["etfs_json"] or "[]"), "created_at": row["created_at"],
...
def refresh_stock_analysis_snapshots() -> dict[str, Any]:
    ...
    for symbol in symbols:
        try:
            ...
            rows_to_insert.append((
                symbol, "us-stock", analysis_id, analysis["current_price"], "USD",
                ...
                analysis["summary"], json.dumps(analysis, ensure_ascii=True),
                json.dumps([], ensure_ascii=True), created_at,
            ))
            inserted += 1
        except Exception as exc:
    ...
    try:
        if rows_to_insert:
            cursor.executemany(
                "INSERT INTO stock_analysis_snapshots (symbol, market, analysis_id, current_price, currency, signal, signal_score, trend_status, support_levels_json, resistance_levels_json, bullish_factors_json, risk_factors_json, summary_text, analysis_json, news_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                rows_to_insert,
            )
    ...
    delete_pattern(_cache_key("overview"))
    return {"inserted_symbols": inserted, "errors": errors, "created_at": created_at}
...
def _get_stock_analysis_snapshot_payload(symbol: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not row:
            ...
            return payload
        payload = {
            "available": True, "symbol": row["symbol"], "market": row["market"],
            "analysis_id": row["analysis_id"], "current_price": row["current_price"],
            "currency": row["currency"], "signal": row["signal"],
            "signal_score": row["signal_score"], "trend_status": row["trend_status"],
            ...
            "summary": row["summary_text"], "analysis": json.loads(row["analysis_json"] or "{}"),
            "created_at": row["created_at"],
        }
        set_json(cache_key, payload, ttl_seconds=STOCK_ANALYSIS_CACHE_TTL_SECONDS)
        return payload
...
def get_stock_analysis_history_payload(symbol: str, limit: int = 10) -> dict[str, Any]:
    ...
    try:
        cursor.execute(
            "SELECT analysis_id, signal, signal_score, trend_status, summary_text, analysis_json, created_at FROM stock_analysis_snapshots WHERE symbol = ? AND market = 'us-stock' ORDER BY created_at DESC, id DESC LIMIT ?",
            (symbol, normalized_limit),
        )
        rows = cursor.fetchall()
        payload = {
            "available": bool(rows), "symbol": symbol,
            "history": [{
                "analysis_id": row["analysis_id"], "signal": row["signal"],
                "signal_score": row["signal_score"], "trend_status": row["trend_status"],
                "summary": row["summary_text"], "analysis": json.loads(row["analysis_json"] or "{}"),
                "created_at": row["created_at"],
            } for row in rows],
...
def get_featured_stock_analysis_payload(limit: int = 6) -> dict[str, Any]:
    ...
    payload = {"available": True, "items": [_get_stock_analysis_snapshot_payload(symbol) for symbol in symbols]}
...
def get_market_news_payload(category: Optional[str] = None, limit: int = 5) -> dict[str, Any]:
    ...
    for category_key in requested_categories:
        ...
        if not snapshot:
            sections.append({
                "category": category_key, "label": definition["label"], "label_zh": definition["label_zh"],
                "description": definition["description"], "description_zh": definition["description_zh"],
                "items": [], "summary": {"category": category_key, "item_count": 0, "activity_level": "unavailable"},
                "created_at": None, "available": False,
            })
            continue
        sections.append({
            "category": category_key, "label": definition["label"], "label_zh": definition["label_zh"],
            ...
            "summary": snapshot["summary"], "created_at": snapshot["created_at"], "available": True,
        })
    ...
    payload = {"categories": sections, "last_updated_at": last_updated_at, "total_items": total_items, "available": any(section.get("available") for section in sections)}
...
def get_market_intel_overview() -> dict[str, Any]:
    ...
    payload = {
        ...
        "categories": [
            {
                "category": section["category"], "label": section["label"],
