...
def test_store_signals_capped():
    ...
    signals = [QuantSignal(type=SignalType.TREND, direction=SignalDir.BULLISH, strength=0.5, confidence=0.5, symbol="A", timeframe="1h", timestamp=datetime.now(timezone.utc)) for _ in range(10)]
...
def test_store_signals_keeps_last():
    ...
    engine._store_signals([QuantSignal(type=SignalType.TREND, direction=SignalDir.BULLISH, strength=0.5, confidence=0.5, symbol="A", timeframe="1h", timestamp=datetime.now(timezone.utc), metadata={"i": 1})])
    engine._store_signals([QuantSignal(type=SignalType.TREND, direction=SignalDir.BULLISH, strength=0.5, confidence=0.5, symbol="A", timeframe="1h", timestamp=datetime.now(timezone.utc), metadata={"i": 2})])
...
def test_last_signals_returns_copy():
    engine = _TestEngine()
    signals = [QuantSignal(type=SignalType.TREND, direction=SignalDir.BULLISH, strength=0.5, confidence=0.5, symbol="A", timeframe="1h", timestamp=datetime.now(timezone.utc))]
    engine._store_signals(signals)
...
def test_update_appends_bar():
    engine = _TestEngine()
    bar = Bar(symbol="TEST", timestamp=datetime.now(timezone.utc), open=100.0, high=101.0, low=99.0, close=100.5, volume=1000)
