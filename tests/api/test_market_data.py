...
def test_quote_not_found(client):
    with patch("api.routes.market_data._get_finnhub") as mock_fh:
        ...
        resp = client.get("/market/quote/INVALID")
...
def test_watchlist_empty(client, mock_session):
    ...
    resp = client.get("/market/watchlist")
    assert resp.status_code == 200
    data = resp.json()
...
def test_watchlist_add(client, mock_session):
    ...
    resp = client.post("/market/watchlist", json={"symbol": "AAPL", "company": "Apple Inc."})
    assert resp.status_code == 200
    data = resp.json()
    assert "watchlist" in data
...
def test_watchlist_add_missing_symbol(client):
    resp = client.post("/market/watchlist", json={})
...
def test_watchlist_remove(client, mock_session):
    ...
    resp = client.delete("/market/watchlist/AAPL")
    assert resp.status_code == 200
...
def test_watchlist_check(client, mock_session):
    mock_session.execute.return_value.scalar_one_or_none.return_value = None
    resp = client.get("/market/watchlist/check/AAPL")
    assert resp.status_code == 200
...
def test_alerts_create(client, mock_session):
    ...
    resp = client.post("/market/alerts", json={"symbol": "AAPL", "target_price": 200, "condition": "ABOVE"})
    assert resp.status_code == 200
    data = resp.json()
...
def test_alerts_create_invalid_condition(client):
    resp = client.post("/market/alerts", json={"symbol": "AAPL", "target_price": 200, "condition": "INVALID"})
    assert resp.status_code == 400


def test_alerts_delete(client, mock_session):
    mock_session.execute.return_value.scalar_one_or_none.return_value = None
    resp = client.delete("/market/alerts/1")
    assert resp.status_code == 200
