...
class KucoinSpotClient(BaseRestClient):
    ...
    def _ensure_ccxt(self):
        if self._ccxt is None:
            import ccxt
            self._ccxt = ccxt.kucoin({"apiKey": self.api_key, "secret": self.api_secret, "password": self.passphrase, "enableRateLimit": True})
...
class KucoinFuturesClient(KucoinSpotClient):
    def _ensure_ccxt(self):
        if self._ccxt is None:
            import ccxt
            self._ccxt = ccxt.kucoinfutures({"apiKey": self.api_key, "secret": self.api_secret, "password": self.passphrase, "enableRateLimit": True})
