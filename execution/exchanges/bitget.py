...
class BitgetMixClient(BaseRestClient):
    def __init__(self, api_key: str = "", api_secret: str = "", passphrase: str = "", config: Optional[Dict] = None):
    ...
    def _ensure_ccxt(self):
        if self._ccxt is None:
            import ccxt
            self._ccxt = ccxt.bitget({"apiKey": self.api_key, "secret": self.api_secret, "password": self.passphrase, "enableRateLimit": True})
...
class BitgetSpotClient(BitgetMixClient):
    def _ensure_ccxt(self):
        if self._ccxt is None:
            import ccxt
            self._ccxt = ccxt.bitget({"apiKey": self.api_key, "secret": self.api_secret, "password": self.passphrase, "options": {"defaultType": "spot"}, "enableRateLimit": True})
