...
def ts_rank(df: pd.DataFrame, n: int) -> pd.DataFrame:
    ...
    Warmup (first n-1 rows per column) returns NaN. Result is a percentile in [0,1].
    """
    if n < 1:
...
def ts_corr(x: pd.DataFrame, y: pd.DataFrame, n: int) -> pd.DataFrame:
    ...
    Constant series in the window -> NaN. Columns missing from either side become NaN.
    """
    if n < 2:
...
def ts_argmax(df: pd.DataFrame, n: int) -> pd.DataFrame:
    """Rolling argmax (0-based index into window), NaN-aware. Warmup -> NaN."""
    if n < 1:
...
def ts_argmin(df: pd.DataFrame, n: int) -> pd.DataFrame:
    """Rolling argmin (0-based index into window), NaN-aware. Warmup -> NaN."""
    if n < 1:
...
def delta(df: pd.DataFrame, d: int) -> pd.DataFrame:
    """First difference at lag d: df - df.shift(d). Lookahead ban: d >= 1."""
    if d < 1:
...
def vwap(panel: dict[str, pd.DataFrame], market: Market | str) -> pd.DataFrame:
    ...
    - equity_cn: (amount * 1000) / (volume * 100 + 1) — Tushare conventions
    - equity_us / equity_hk / futures: typical price (H+L+C+O)/4 when panel["vwap"] absent
