...
import { drawControlHandle, pointInRect } from '../Utils'

export class Rectangle extends DrawingTool {
  static readonly pointCount = 2
  get pointCount() { return Rectangle.pointCount }
  ...
  hitTest(x: number, y: number, mapper: any, paneIndex: number): boolean {
    ...
    return pointInRect(x, y, Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1), Math.abs(y2 - y1))
  }
}
