...
export async function searchSymbols(q: string, exchange?: string, limit?: number): Promise<unknown[]> {
...
export async function getSymbolInfo(symbol: string, exchange: string): Promise<unknown> {
...
export async function setFreezeQty(symbol: string, exchange: string, qty: number): Promise<unknown> {
  const { data } = await api.post('/openalgo/symbols/freeze-qty', { symbol, exchange, qty })
  return data
}

export async function getMarketHolidays(year?: number): Promise<unknown> {
  const { data } = await api.get('/openalgo/symbols/holidays', { params: { year: year || new Date().getFullYear() } })
  return data
}

export async function getMarketTimings(date?: string): Promise<unknown> {
