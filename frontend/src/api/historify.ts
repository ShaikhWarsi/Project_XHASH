...
export async function fetchOHLCV(symbol: string, exchange: string, timeframe: string, from?: string, to?: string): Promise<any> {
...
export async function downloadHistory(symbol: string, exchange: string, timeframe: string, from: string, to: string): Promise<any> {
  const { data } = await api.post('/openalgo/historify/download', { symbol, exchange, timeframe, from_date: from, to_date: to })
  return data
}

export async function fetchDownloadJobs(): Promise<any> {
...
export async function addToWatchlist(symbol: string, exchange: string): Promise<any> {
...
export async function exportCSV(symbol: string, exchange: string, timeframe: string): Promise<any> {
...
export async function createSchedule(symbol: string, exchange: string, timeframe: string, type: string, time: string): Promise<any> {
