...
export async function updateWhatsAppConfig(config: any): Promise<any> {
  const { data } = await api.post('/openalgo/whatsapp/config', config)
  return data
}

export async function pairWhatsApp(phone: string): Promise<any> {
  const { data } = await api.post('/openalgo/whatsapp/pair', { phone })
  return data
}

export async function getPairingStatus(): Promise<any> {
  const { data } = await api.get('/openalgo/whatsapp/pair/status')
  return data
}

export async function unlinkWhatsApp(): Promise<any> {
...
export async function stopWhatsAppBot(): Promise<any> {
  const { data } = await api.post('/openalgo/whatsapp/bot/stop')
  return data
}

export async function getWhatsAppBotStatus(): Promise<any> {
  const { data } = await api.get('/openalgo/whatsapp/bot/status')
  return data
}

export async function getWhatsAppUsers(): Promise<any[]> {
  const { data } = await api.get('/openalgo/whatsapp/users')
  return data.users || []
}

export async function unlinkWhatsAppUser(jid: string): Promise<any> {
  const { data } = await api.post(`/openalgo/whatsapp/users/${encodeURIComponent(jid)}/unlink`)
  return data
}

export async function sendWhatsAppMessage(jid: string, text: string): Promise<any> {
  const { data } = await api.post('/openalgo/whatsapp/send', { jid, text })
  return data
}

export async function broadcastWhatsApp(text: string): Promise<any> {
  const { data } = await api.post('/openalgo/whatsapp/broadcast', { text })
  return data
}

export async function getWhatsAppStats(): Promise<any> {
